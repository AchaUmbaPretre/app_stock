    ...
    const user = useSelector((state) => state.user?.currentUser);
      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            const { data } = await axios.get(`${DOMAIN}/api/vente/vente/detteJour`);
            setData(data);
            setLoading(false)
          } catch (error) {
            console.log(error);
            ...
            <div className="product-container">
                <div className="product-container-top">
                    <div className="product-left">
                        <h2 className="product-h2" style={{fontSize: '22px'}}>Ventes à crédit du jour</h2>
                        <span></span>
                    </div>
