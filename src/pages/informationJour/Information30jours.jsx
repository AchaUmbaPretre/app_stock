...
const Information30Jours = ({ period }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const { data } = await axios.get(`${DOMAIN}/api/vente/venteDuJour30`);
            setData(data);
          } catch (error) {
            console.log(error);
          }
      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            const { data } = await axios.get(`${DOMAIN}/api/client/clientCount/countJour30`);
...
export default Information30Jours