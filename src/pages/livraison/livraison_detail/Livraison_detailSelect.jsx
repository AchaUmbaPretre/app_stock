...
const Livraison_detailSelect = ({getProduits}) => {
  ...
  const handleClick = async (e) => {
   ...
   if (!datas.start_date || !datas.end_date ) {
    ...
    try {
      const {data} = await axios.get(`${DOMAIN}/api/livraison/livraisonDetail?start_date=${datas.start_date}&end_date=${datas.end_date}`);
      getProduits(data)
    } catch (err) {
      Swal.fire({
        title: 'Error',
...
export default Livraison_detailSelect