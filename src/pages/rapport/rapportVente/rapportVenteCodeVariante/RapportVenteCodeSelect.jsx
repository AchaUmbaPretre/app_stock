...
const RapportVenteCodeSelect = ({getProduits,id}) => {
  ...
  const handleClick = async (e) => {
   ...
   if (!datas.start_date || !datas.end_date ) {
    ...
    try {
      const {data} = await axios.get(`${DOMAIN}/api/rapport/rapport/venteV/${id}?start_date=${datas.start_date}&end_date=${datas.end_date}`);
      getProduits(data)
    } catch (err) {
      Swal.fire({
        title: 'Error',
...
export default RapportVenteCodeSelect